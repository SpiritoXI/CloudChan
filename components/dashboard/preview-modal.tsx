...
import { ImageViewer, MediaPlayer, PdfViewer, TextViewer } from "@/components/common";
import { isImageFile, isMediaFile, isVideoFile, isAudioFile, isPdfFile, isTextFile, downloadFile } from "@/lib/utils";
...
export function PreviewModal({ file, isOpen, gateways, onClose }: PreviewModalProps) {
  ...
  const getFileIcon = (filename: string) => {
    ...
    if (isPdfFile(filename)) return <FileText className="h-4 w-4 text-red-400" />;
    if (isTextFile(filename)) return <FileText className="h-4 w-4 text-amber-400" />;
  ...
  const handleDownload = useCallback(() => {
    ...
    const gateway = availableGateways.length > 0 ? availableGateways[0] : gateways[0];
    if (gateway) {
      downloadFile(`${gateway.url}${file.cid}`, file.name);
    }
  }, [file, gateways]);
