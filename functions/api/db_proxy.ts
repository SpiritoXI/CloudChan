        ...
        if (Array.isArray(files)) {
          for (let i = 0; i < files.length; i++) {
            try {
              ...
              if (String(file.id) === String(updateBody.fileId)) {
                const originalData = files[i];
                ...
                try {
                  ...
                  console.error("更新文件失败，尝试恢复原始数据:", updateError);
                  try {
                    await upstashCommand(
                      env.UPSTASH_URL,
                      env.UPSTASH_TOKEN,
                      ["LPUSH", FILES_KEY, originalData]
                    );
                  } catch {
                    console.error("恢复原始数据失败");
                  }
                  throw updateError;
                }
                break;
              ...
              console.error("解析文件数据失败:", parseError);
            }
          }
        }
