...
export interface UploadQuota {
  dailyLimit: number;      // 每日上传限制（字节）
  usedToday: number;       // 今日已使用（字节）
  remaining: number;       // 剩余配额（字节）
  lastReset: number;       // 上次重置时间戳
}

const QUOTA_STORAGE_KEY = 'cc_upload_quota_v1';
const DAILY_UPLOAD_LIMIT = 10 * 1024 * 1024 * 1024; // 10GB 每日限制

/**
 * 获取上传配额信息
 * @returns 当前配额状态
 */
export function getUploadQuota(): UploadQuota {
  try {
    ...
    if (stored) {
      const quota: UploadQuota = JSON.parse(stored);
...
/**
 * 更新上传配额
 * @param bytesUsed - 本次使用的字节数
 */
export function updateUploadQuota(bytesUsed: number): void {
  try {
    const quota = getUploadQuota();
    quota.usedToday += bytesUsed;
    quota.remaining = Math.max(0, quota.dailyLimit - quota.usedToday);
...
/**
 * 检查是否有足够配额
 * @param bytesNeeded - 需要的字节数
 * @returns 是否足够
 */
export function hasEnoughQuota(bytesNeeded: number): boolean {
  const quota = getUploadQuota();
 ...
 * @returns 格式化后的字符串
 */
export function formatQuotaInfo(quota: UploadQuota): string {
  const used = formatBytes(quota.usedToday);
  const limit = formatBytes(quota.dailyLimit);
  const remaining = formatBytes(quota.remaining);
  const percentage = Math.round((quota.usedToday / quota.dailyLimit) * 100);
  return `已使用: ${used} / ${limit} (${percentage}%)，剩余: ${remaining}`;
}
