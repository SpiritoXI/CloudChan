...
### 新增功能 ✨
- 📥 **批量 CID 导入功能**
  - 支持单个 CID 导入，可自定义文件名和大小
  - 支持批量 CID 导入，支持逐行输入多个 CID
  - 自动验证 CID 格式（支持 CIDv0 和 CIDv1）
  - 自动去重，避免导入重复的 CID
  - 可选择是否自动获取文件信息（名称和大小）
...
- 🎯 **独立分享页面**
  - 创建白色水晶风格分享页面（share.html）
  - 支持验证分享链接的有效性
  - 显示文件信息（名称、大小、CID、有效期）
  - 支持直接下载文件和复制 CID
  - 分享过期后显示友好提示

### 技术实现
- 新增 `share.html` 和 `share.js` 分享页面
- 新增 `/api/share` API 接口，处理分享链接的创建和访问
- 在 `app.js` 中添加 CID 导入、复制和分享的业务逻辑
- 在 `ui.js` 中添加模态框交互和提示功能
- 在 `index.html` 中添加 CID 按钮和模态框 UI
- 在 `style.css` 中添加模态框和分享页面的样式
